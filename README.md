...
We crop out birds with the images and segmentations in [Caltech-UCSD Birds-200-2011 (CUB)](https://www.vision.caltech.edu/datasets/cub_200_2011) and transfer them onto the high-resolution background images (MD5: 67e186b496a84c929568076ed01a8aa1) from [Places](http://places2.csail.mit.edu) dataset to generate our waterbird dataset. The datasets are loaded from `./CUB` and `./data_large` by default, and the data path can be modified in `generate_waterbird.py`. We then use the pre-trained [ResNet50](https://download.pytorch.org/models/resnet50-19c8e357.pth) in PyTorch to extract features from the concatenated images and store them in `./res`. The data can be generated by the command 
