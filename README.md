...
We will use the CUB and Places dataset to generate our waterbird dataset. The above two datasets can be found at [Google Drive](https://drive.google.com/drive/folders/1cCrbijIyGAOQ_uB8SdQnT43eaB7bi8Qj). The data can be generated by the command `python generate_waterbird.py`,  and the dataset will be saved in `./res`.

Then, you can run `python app2_waterbird.py --method FAIR --nstart 10` to train our FAIR linear logistic regression model on the waterbird dataset 10 times. We also support other methods, including LASSO, IRM, and GroupDRO. You can apply the methods above by changing the instructions. You can check the curve of test accuracy in `./saved_results/waterbird_test_curve.pdf`.
